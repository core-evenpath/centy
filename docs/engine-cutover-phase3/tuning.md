...
tsc baseline = **276** (not 401 as Phase 2 close docs claim).
...
Every Phase 3 commit measures against 276 with:
```
rm -rf .next && rm -f tsconfig.tsbuildinfo && npx tsc --noEmit 2>&1 | grep -c "error TS"
```

The remaining 276 errors are distributed cleanup concerns unrelated to the generator bug. Bringing 276 to zero is **not a Phase 3 prerequisite** — it's separate work.
