    ...
    return (
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-gray-50">
            <div className="relative mb-8">
                <div className="relative w-20 h-20 bg-white rounded-2xl flex items-center justify-center border border-gray-200">
                    <Inbox className="w-10 h-10 text-gray-400" />
                </div>
                <div className="absolute -bottom-2 -right-2 w-10 h-10 bg-[#111] rounded-xl flex items-center justify-center">
            ...
            <div className="flex items-center gap-4 mb-8 px-5 py-3 bg-white rounded-xl border border-gray-200">
                <div className="flex items-center gap-2">
                    {isWhatsAppConnected === null ? (
                        <div className="w-2.5 h-2.5 rounded-full bg-gray-200 animate-pulse" />
                    ) : (
                        <div className={`w-2.5 h-2.5 rounded-full ${isWhatsAppConnected ? 'bg-green-500' : 'bg-gray-300'}`} />
                    )}
                ...
                <div className="flex items-center gap-2">
                    {isTelegramConnected === null ? (
                        ...
                        <div className={`w-2.5 h-2.5 rounded-full ${isTelegramConnected ? 'bg-blue-500' : 'bg-gray-300'}`} />
                    )}
                    <span className="text-sm text-gray-600">
            ...
            <div className="flex flex-col gap-3 w-full max-w-xs">
                ...
                {!isLoading && !isWhatsAppConnected && (
                    <Link href="/partner/apps/whatsapp-api" className="w-full">
                        <Button
                            className={`w-full h-11 rounded-xl ${isPending
                ...
                {!isLoading && !isTelegramConnected && (
                    <Link href="/partner/apps/telegram-api" className="w-full">
                        <Button
                            className="w-full h-11 rounded-xl bg-blue-600 hover:bg-blue-700 text-white flex items-center justify-center gap-2 transition-colors"
                        >
                ...
                <Link href="/partner/apps" className="w-full">
                    <Button variant="outline" className="w-full h-11 rounded-xl border-gray-200 text-gray-600 hover:bg-gray-100 hover:text-gray-900 transition-colors bg-white">
