    ...
    <>
                    ...
                    <div className="relative">
                      ...
                      >
                        {fetchedTickers.map(ticker =>
                          <option key={ticker} value={ticker} selected={formData.ticker === ticker}>{ticker}</option>)}
                      </select>
      ...
      <Dialog open={showDataModal} onOpenChange={setShowDataModal}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4 max-h-96 overflow-scroll">
            ...
            <div>
              ...
              <ul className="list-disc pl-5 mt-1 space-y-1 text-sm">
                {fetchedStockData?.aiThesis?.map((item, index) => <li key={index}>{item}</li>)}
              </ul>
            </div>
            <div>
              <strong>Catalysts:</strong>
              <ul className="list-disc pl-5 mt-1 space-y-1 text-sm">
                {fetchedStockData?.aiCatalysts?.map((item, index) => <li key={index}>{item}</li>)}
              </ul>
            </div>
            <div>
              <strong>Key Risks:</strong>
              <ul className="list-disc pl-5 mt-1 space-y-1 text-sm">
                {fetchedStockData?.aiRisks?.map((item, index) => <li key={index}>{item}</li>)}
              </ul>
            </div>
          </div>
          <DialogFooter>
