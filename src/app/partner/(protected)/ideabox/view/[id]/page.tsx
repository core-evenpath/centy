              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    <Badge variant="secondary" className="text-xs font-semibold">{idea.sector}</Badge>
                    <Badge variant="secondary" className="text-xs font-semibold capitalize">{idea.ideaType}</Badge>
                  </div>
                  <h1 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-2">{idea.companyName}</h1>
                  <p className="text-2xl font-mono font-semibold text-gray-700">{idea.ticker}</p>
                </div>
                <div className="flex items-center justify-center lg:justify-end">
                  <div className={`${actionBadgeColor} rounded-2xl px-8 py-6 text-center shadow-lg`}>
                    ...
                    <div className="text-white text-4xl font-bold uppercase">{idea.action}</div>
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-8 pt-6 border-t border-current/10">
                <DetailItem icon={Target} label="Price Target" value={<span className="text-xl">{idea.priceTarget}</span>} />
                <DetailItem icon={DollarSign} label="Current Price" value={<span className="text-xl">{idea.currentPrice || 'N/A'}</span>} />
                <DetailItem icon={Clock} label="Timeframe" value={<span className="text-xl capitalize">{idea.timeframe}</span>} />
                <DetailItem icon={Shield} label="Risk Level" value={<span className={`text-xl capitalize font-bold ${riskColor}`}>{idea.riskLevel}</span>} />
              </div>
            </div>
          </div>
            ...
            <div className="space-y-6">
                ...
                {idea.imageUrl ? (
                  <div className="space-y-4">
                    <div className="relative overflow-hidden rounded-lg border-2 border-gray-200 shadow-md hover:shadow-lg transition-shadow">
                      <Image src={idea.imageUrl} alt={`${idea.ticker} Recommendation Card`} width={400} height={225} className="w-full h-auto" />
                    </div>
                    <p className="text-xs text-gray-500 text-center">This card can be shared with clients</p>
                  </div>
                ) : (
              ...
              <Section title="Tracking Info" icon={Calendar}>
                <div className="space-y-4">
                  ...
                  <DetailItem icon={Calendar} label="Created" value={idea.createdAt ? new Date((idea.createdAt as any).toDate()).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' }) : 'N/A'} />
                  <DetailItem icon={Calendar} label="Last Updated" value={idea.updatedAt ? new Date((idea.updatedAt as any).toDate()).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' }) : 'N/A'} />
                </div>
              </Section>
            </div>
          </div>
