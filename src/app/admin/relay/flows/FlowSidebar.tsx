...
export default function FlowSidebar({ selectedId, onSelect, isPlaying, onTogglePlay, onReset }: Props) {
  ...
  // Find selected sub-vertical info
  const selectedSub = selectedId
    ? VERTICALS.flatMap(v => v.subVerticals).find(s => s.id === selectedId)
    : null;
    ...
    <div style={{ width: 300, borderRight: `1px solid ${T.bdrL}`, background: T.surface, display: 'flex', flexDirection: 'column', height: '100vh', flexShrink: 0 }}>
          ...
          return (
            <div key={v.id} style={{ marginBottom: 2 }}>
              <button onClick={() => toggle(v.id)} style={{
                ...
                padding: '8px 14px', border: 'none', background: 'transparent', cursor: 'pointer', textAlign: 'left',
              }}>
              ...
              {isOpen && subs.map(s => (
                <button key={s.id} onClick={() => onSelect(s.id)} style={{
                  ...
                  {CURATED_IDS.has(s.id) && (
                    <span style={{ fontSize: 9, fontWeight: 600, color: '#16a34a', background: 'rgba(22,163,74,0.08)', padding: '1px 6px', borderRadius: 4 }}>curated</span>
                  )}
      ...
      {selectedSub && (
        <div style={{ padding: 14, borderTop: `1px solid ${T.bdrL}` }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: T.t1, marginBottom: 4 }}>{selectedSub.name}</div>
          <div style={{ fontSize: 10, color: T.t4, marginBottom: 10 }}>{selectedSub.blocks.length} blocks</div>
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button onClick={onReset} style={{
              padding: '9px 14px', borderRadius: 8, border: `1px solid ${T.bdrL}`,
              background: T.surface, cursor: 'pointer', color: T.t3, display: 'flex', alignItems: 'center',
            }}>
              <RotateCcw size={12} />
            </button>
          </div>
